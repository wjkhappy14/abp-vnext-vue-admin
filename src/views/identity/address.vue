<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在加载..." border fit highlight-current-row>
      ...
      <el-table-column label="国家名称" width="180" align="center">
        <template slot-scope="scope">
          <el-tag>{{ scope.row.countryName }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="省/州" width="180" align="center">
        <template slot-scope="scope">
          {{ scope.row.stateProvinceName }}
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="地址类型" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.addressType}}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="详细地址" width="200">
        <template slot-scope="scope">
