  ...
  <div class="app-container">
    <el-tabs v-model="activeName" @tab-click="tabClickHandler">
      <el-tab-pane label="Client" name="client">
        <el-table v-loading="listLoading" :data="identityResource.items" element-loading-text="Loading..." border fit highlight-current-row>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        identityResource: 'identityServer/identityResource/identityResource'
      }),
      ...mapState({
        users: state => state
      })
    ...
    methods: {
      ...mapActions({
        getIdentityResources: "identityServer/identityResource/getIdentityResources"
      }),
      tabClickHandler() {
